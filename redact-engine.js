...
const ANCH=[["title",`${BD}(?:${TITLES})[,\\s]+(${NME})`,"מופיע אחרי תואר"],
 ...
 ["carep",`${BD}(?:[בהולמכש]|וה|ול|ש)?(?:מורה|גננת|מטפלת|מטפל|מדריכה|מדריך|מנהלת|מנהל|פסיכולוגית|פסיכולוג|רופאה|רופא|אחות|קלינאית|עו"ס|עובדת\\s+סוציאלית|עובד\\s+סוציאלי)\\s+(${NME})`,
  ...
  for(const a of ANCH){a.rx.lastIndex=0;let m;
    while((m=a.rx.exec(n))){
      for(let g=1;g<m.length;g++){
        ...
        if(a.k==="carep"){const f=norm(c.split(/\s+/)[0]);
