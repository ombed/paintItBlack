...
(async()=>{
 ...
 // מיזוג אינו אוטומטי (Q12 בגרסה 2): בלי אישור שלה אלה שני אנשים; עם sameAs — אחד
 ({out,map}=await run(["שלוה ליבוביץ מסרה. שלווה ליבוביץ הוסיפה."],["שלוה ליבוביץ","שלווה ליבוביץ"]));
 ok(map["שלוה ליבוביץ"]!==map["שלווה ליבוביץ"],"בלי אישור — שני כינויים, לא מיזוג שקט");
 ({out,map}=await run(["שלוה ליבוביץ מסרה. שלווה ליבוביץ הוסיפה."],[{value:"שלוה ליבוביץ",kind:"NAME",replacement:"",sameAs:"שלווה ליבוביץ"},{value:"שלווה ליבוביץ",kind:"NAME",replacement:""}]));
 console.log("   שלוה → "+map["שלוה ליבוביץ"]+"  |  שלווה → "+map["שלווה ליבוביץ"]);
 ok(map["שלוה ליבוביץ"]===map["שלווה ליבוביץ"],"עם אישור — שני הכתיבים קיבלו את אותו שם בדוי");
 ...
 console.log("   "+out);
 // ההחלטה שלה (Q14 בגרסה 2): מה שהיא הקלידה נשאר, וההתנגשות מסומנת — לא מוחלף בשקט בשם אחר.
 const both=(out.match(/יעל רוזן/g)||[]).length;
 ok(both===3&&!out.includes("רונית לוי"),"השם שהיא בחרה נשאר, גם כשהוא מתנגש עם יעל רוזן האמיתית — נמצאו "+both+" מופעים");
 ...
 // הגוף מקבל שם בדוי ("הגפן"), ואחרי ב/ל הוא נכתב בלי ה ("בגפן"). ההחזרה חייבת לתפוס גם את הצורה הזאת.
 rr=E.restoreNames(out,[["פנים מאירות",map["פנים מאירות"]]]);
 ok(rr.text.includes("בפנים מאירות")&&rr.text.includes("לפנים מאירות"),"והשם הבדוי חוזר לשם הגוף עם אות השימוש: "+rr.text);
