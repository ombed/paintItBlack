...
console.log("\n— a confirmed short name covers its prefixed forms (Q3) —");
{
  ...
  let ex = null;
  for (const n of ["שם", "אור", "גל", "טל", "בר", "עד", "לב", "דן", "חן", "רם", "נר"]) for (const p of "בלמושהכ") if (!ex && isWord(p + n) && !isWord(n) === false) ex = [n, p + n];
  if (!ex) for (const n of ["שם", "אור", "גל", "טל", "בר", "עד", "לב", "דן", "חן", "רם", "נר"]) for (const p of "בלמושהכ") if (!ex && isWord(p + n)) ex = [n, p + n];
  if (ex) {
...
console.log("\n— fold evidence from the model (Q4) —");
{
  ...
  return C.foldEvidence(pipe, out, TEXT).then(() => {
    ...
    return C.foldEvidence({}, out2, TEXT).then(() => {
...
function part2() {
  ...
  ok(C.fakeDate("31.2.2026", 10) === null, "an impossible date is not shifted");
  {
    ...
    const other = hits("אחר לגמרי. 11.2.2026.", [], { ...DOPT, styles: { "*": "blank", DATE: "name" } }).filter((x) => x.type === "DATE");
    ok(other[0].rep !== out[0].rep || true, "another document may differ");
