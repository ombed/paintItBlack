...
{
  // nikud between the pieces: "##ד" sits one past the TIMEX, so nerGroup does not attach it
  const text = "דוִד";
  const rows = toks([["דו", "B-TIMEX"], ["##ד", "B-PER"]]);
  E.nerAlign(text, rows, 0);
  ok(rows[1]._s === 3, "the piece is placed after the mark: " + rows[1]._s);
  const a = adapt(rows, DICTA);
  ok(labels(a) === "O B-PER", "a detached piece is not swallowed: " + labels(a));
