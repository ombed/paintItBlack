...
/* A pretend token classifier. lex: word -> label, or -> [[piece, label], ...] for a word the
   ...
   with the chunk in the message, as ORT errors can. opt.drop: a word whose token comes back
   spelled differently (nikud stripped), so it cannot be aligned. opt.long: token count to
   report for any chunk containing that word. */
  ...
  const pipe = async (t, args) => {
    ...
    for (const w of split(t)) {
      ...
      for (const [p, label] of parts) {
        const word = o.drop && w === o.drop ? p.replace(/[֑-ׇ]/g, "") : p;
...
(async () => {
  ...
  console.log("\n— health: long chunks, unmapped labels, tokens that cannot be placed —");
  {
    ...
    ok(r.health.alignFailTokens === 1, "the nikud-stripped token is an alignment failure: " + r.health.alignFailTokens);
