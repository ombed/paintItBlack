...
בכל פריסה לקדם את שני המקומות **יחד**:

- `index.html` — המופעים של `v8` בבלוק הפתיחה (השבב התחתון וה-console)
- `sw.js` — `const V="hedact-v8"`

אם יישכח אחד מהם, השבב התחתון יציג «מוגש v7 — רענון בלי מטמון» ויסגיר
